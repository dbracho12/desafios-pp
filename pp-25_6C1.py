...
def iniciar():
    try:
        ...
        # 2) Utilizar requests para consumir toda la información
        response = db.request.get("https://jsonplaceholder.typicode.com/todos")
        data = response.json()
        ...
        for dato in data:
            dato_response= {}
            dato_response['userId'] = dato.userId
            dato_response['titulo'] = dato.titulo
            dato_response['completedo'] = dato.completado
            datos.append(dato_response)    
